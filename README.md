...
[Open the public pilot](https://wooden-dogfish-387.convex.site). The frontend is hosted on Convex; it currently uses the dedicated cloud development deployment. [Vibe Apps submission](https://vibeapps.dev/s/one-table) · [Playable walkthrough](https://drive.google.com/file/d/19CIu9tWAfKNo1nFfHzACgcnQIv-oUi_C/view) · [Build record](hackathon.md) · [Architecture](docs/architecture.md).
...
The latest run with the current extraction prompt completed five of twelve cases before persistent gateway rate limits: **65/65 scored fields, 35/35 blank-when-unstated checks, and 5/5 totals**. The complete twelve-case gate has not passed. `experiments/extraction/results-openai.json` records the prompt hash, source revision, scores and errors. The prior partial run is preserved separately. The example board uses Claude’s historical fictional results and says so.

PDF, PNG and JPEG quotes can now be uploaded manually (4 MiB maximum), stored privately and extracted by OpenAI. Controlled PDF and PNG tests both calculated INR 141,600 for 80 guests at INR 1,500 plus 18% GST. Firecrawl published-pricing extraction and automatic inbound email attachment ingestion remain unfinished. Answers to requested clarification terms can complete a prior quote while retaining source-message IDs. New pricing and declines do not use that merge. Review the original thread before booking. Bookings and contract negotiation remain with the buyer.

## Try document extraction

Use the [fictional PDF or image](docs/evidence/README.md) with the documented expected total. No venue email is needed.
...
Configure the backend through Convex, using your own provider accounts:

- `SITE_URL`: the frontend origin for sign-in emails. Use `http://localhost:5190` locally; change it to your public frontend origin before deployment.
- `FIRECRAWL_API_KEY` and either `OPENAI_API_KEY` or `AI_GATEWAY_API_KEY`.
- `AGENTMAIL_API_KEY` and `AGENTMAIL_INBOX_ID`: a dedicated inbox with send/read and webhook access.
- `JWT_PRIVATE_KEY` and `JWKS`: generated by the Convex Auth setup command above.
- Set `PUBLIC_SENDING_ENABLED=false` during initial setup; enable it only when your inbox and limits are configured.

Set `WEBHOOK_ORIGIN` in your shell to your public `https://your-deployment.convex.site` origin, then run `node scripts/configure-inbox.mjs`. It registers the inbound webhook and stores its signing secret in Convex. `WEBHOOK_ORIGIN` is separate from `SITE_URL` because a local frontend cannot receive provider webhooks. The script sends no email. See `.env.example` for names; never put provider secrets in frontend variables.
