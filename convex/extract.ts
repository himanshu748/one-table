...
export const fromMessage = internalAction({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      await ctx.runMutation(internal.limits.reserveExtraction, {
        messageId: ctxData.messageId,
      });
      const content: Array<unknown> = [
        {
          type: "text",
          text: `Buyer asked for a quote for ${ctxData.headcount} guests on ${ctxData.eventDate}.\n\nSubject: ${ctxData.subject}\n\n${ctxData.body}`,
        },
      ];
      for (const id of ctxData.attachmentIds.slice(0, 1)) {
        ...
        content.push(
          blob.type === "application/pdf"
            ? { type: "file", file: { filename: "quote.pdf", file_data: data } }
