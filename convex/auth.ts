...
const EmailCode = Email({
  ...
  async sendVerificationRequest({identifier, token}, ctx?: ActionCtx) {
    ...
    const result = await fetch(`https://api.agentmail.to/v0/inboxes/${encodeURIComponent(inbox)}/messages/send`, {
      method: "POST", signal: AbortSignal.timeout(30000),
      headers: {"content-type":"application/json", authorization:`Bearer ${key}`},
      body: JSON.stringify({to:[email], subject:"Your One Table sign-in code", text:`Your One Table sign-in code is:\n\n${token}\n\nEnter it only at https://wooden-dogfish-387.convex.site/#workspace. It expires in 10 minutes. Never share this code. If you did not request it, ignore this email.`}),
    });
    if (!result.ok) throw new Error("We could not deliver the sign-in email. Please try again later.");
  },
});
export const { auth, signIn, signOut, store, isAuthenticated } = convexAuth({
  ...
  callbacks: {
    afterUserCreatedOrUpdated: async (authCtx, args) => {
      ...
      const events = await ctx.db.query("events").withIndex("by_user", q=>q.eq("userId",guestId)).take(1000);
      for (const event of events) await ctx.db.patch(event._id, {userId:args.userId});
    },
  },
});
