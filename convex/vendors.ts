...
export const recordOutbound = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(vendor._id, {
      status: args.status,
      ...(args.status === "followup_sent" ? { followupState: "sent", followupError: undefined } : { outboundState: "sent" }),
...
export const setAutoFollowup = mutation({
  args: {vendorId: v.id("vendors"), enabled: v.boolean()}, returns: v.null(),
...
export const claimFollowup = internalMutation({
  args: {vendorId: v.id("vendors"), messageId: v.id("messages")}, returns: v.boolean(),
  handler: async (ctx, {vendorId, messageId}) => {
    const vendor = await ctx.db.get(vendorId);
    if (!vendor || vendor.followupState !== "queued") return false;
    const live = await ctx.db.query("quotes").withIndex("by_vendor_live", q=>q.eq("vendorId",vendorId).eq("supersededAt",null)).first();
    const event = await ctx.db.get(vendor.eventId);
    if (!vendor.autoFollowup || !event || !(await canSend(ctx, event.userId, vendor.email)) || live?.messageId !== messageId || live.reply_kind !== "quote") {
      await ctx.db.patch(vendorId,{followupState:"cancelled"});
      return false;
    }
    const quota = await limits.limit(ctx, "mailGlobal");
    if (!quota.ok) {
      await ctx.db.patch(vendorId,{followupState:"cancelled",followupError:"Daily sending limit reached. Review the reply manually."});
      return false;
    }
